...
sudo wget https://download.sonatype.com/nexus/3/nexus-3.65.0-02-unix.tar.gz > /dev/null || log_and_exit "Failed to download Nexus."
log "Extracting Nexus..."
sudo tar -xzvf nexus-3.65.0-02-unix.tar.gz > /dev/null || log_and_exit "Failed to extract Nexus."

# Remove downloaded archive
log "Removing downloaded archive..."
sudo rm -rf nexus-3.65.0-02-unix.tar.gz || log_and_exit "Failed to remove downloaded archive."
log "Renaming extracted directory..."
sudo mv nexus-3.65.0-02 nexus || log_and_exit "Failed to rename extracted directory."
)
